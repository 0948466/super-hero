...
CKEDITOR.dialog.add('image2', editor => {
  ...
  let regexGetSizeOrEmpty = /(^\s*(\d+)(px)?\s*$)|^$/i,
  ...
  function validateDimension() {
    let match = this.getValue().match(regexGetSizeOrEmpty),
  ...
  function createPreLoader() {
    ...
    return function(src, callback, scope) {
      addListener('load', () => {
        ...
        const dimensions = getNatural(image);
      ...
      image.setAttribute(
        'src',
        `${(config.baseHref || '') + src}?${Math.random()
          .toString(16)
          .substring(2)}`,
      )
    }
  ...
  function toggleLockRatio(enable) {
    ...
    if (CKEDITOR.env.hc) {
      const icon = lockButton.getChild(0);
